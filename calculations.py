...
def calculate_roi(loan_amount: float, original_rate: float, buydown_rate: float, 
    ...
    """PRD 3.2.3 - Unified implementation"""
    # Get prices from external data
    ...
    with engine.connect() as conn:
        conn.execute(
            text("""
                INSERT INTO daily_roi (date, coupon_rate, price, roi, breakeven_months)
                VALUES (:date, :rate, :price, :roi, :breakeven)
            """),
            [{"date": date, "rate": original_rate, "price": price1, 
